...
def test(args: Namespace, eval_ds: Dataset, real_model, synt_model,
    ...
    with torch.no_grad():
        ...
        queries_subset_ds = Subset(eval_ds, list(range(eval_ds.num_db, eval_ds.num_db+eval_ds.num_q)))
    ...
    for positive_dist_threshold in [10, 25, 50, 100, 200, 500, 1000]:
        ...
        if num_preds_to_save != 0 and positive_dist_threshold == 100:
            ...
            visualizations.save_preds(predictions[:, :num_preds_to_save], eval_ds, args.log_dir,
                                      args.save_only_wrong_preds, preds_folder_name=preds_folder_name,
