...
def write_labels_to_image(labels=["text1", "text2"]):
    ...
    img = Image.new(
        "RGB", ((W * len(labels)) + 50 * (len(labels) - 1), TEXT_H), (1, 1, 1)
    )
...
def draw(img, c=(0, 255, 0), thickness=20):
    ...
    return cv2.line(
        img, (p[3, 0], p[3, 1]), (p[0, 0], p[0, 1]), c, thickness=thickness * 2
    )
...
def build_prediction_image(images_paths, preds_correct=None):
    ...
    labels = ["Query"] + [
        f"Pr{i} - {is_correct}" for i, is_correct in enumerate(preds_correct[1:])
    ]
...
def save_preds(
    ...
    predictions : np.array of shape [num_queries x num_preds_to_viz], with the preds
        for each query
    eval_ds : TestDataset
    log_dir : str / Path with the path to save the predictions
    ...
    os.makedirs(f"{log_dir}/preds_{preds_folder_name}", exist_ok=True)
    for query_index, preds in enumerate(
        tqdm(predictions, ncols=120, desc=f"Saving preds in {log_dir}")
    ):
        ...
        pred_image_path = f"{log_dir}/preds_{preds_folder_name}/{query_index:03d}.jpg"
        prediction_image.save(pred_image_path)

        positives_paths = [
            eval_ds.database_paths[idx] for idx in positives_per_query[query_index]
        ]
        save_file_with_paths(
            query_path=list_of_images_paths[0],
            preds_paths=list_of_images_paths[1:],
            positives_paths=positives_paths,
            output_path=f"{log_dir}/preds_{preds_folder_name}/{query_index:03d}.txt",
        )
